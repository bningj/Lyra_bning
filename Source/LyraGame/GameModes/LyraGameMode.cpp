...
void ALyraGameMode::OnExperienceLoaded(const ULyraExperienceDefinition* CurrentExperience)
{
	...
	for (FConstPlayerControllerIterator Iterator = GetWorld()->GetPlayerControllerIterator(); Iterator; ++Iterator)
	{
		ALyraPlayerController* PC = Cast<ALyraPlayerController>(*Iterator);
		if (PC != nullptr && PlayerCanRestart(PC)) {
			PC->LocalPlayerNum = this->LocalPlayerNum;
			RestartPlayer(PC);
		}
	}
}
...
APawn* ALyraGameMode::SpawnDefaultPawnAtTransform_Implementation(AController* NewPlayer, const FTransform& SpawnTransform)
{
	...
	{
		/*GEngine->AddOnScreenDebugMessage(0, 3600, FColor::Red, TEXT("ready to spawn"));
		UE_LOG(LogTemp, Warning, TEXT("ready to spawn"));*/
		ALyraPlayerController* PC = Cast<ALyraPlayerController>(NewPlayer);
		for (int32 i = 0; i < PC->LocalPlayerNum; ++i) {
			if (APawn* SpawnedPawn = GetWorld()->SpawnActor<APawn>(PawnClass, SpawnTransform, SpawnInfo))
			{
				if (ULyraPawnExtensionComponent* PawnExtComp = ULyraPawnExtensionComponent::FindPawnExtensionComponent(SpawnedPawn))
				{
					if (const ULyraPawnData* PawnData = GetPawnDataForController(NewPlayer))
					{
						...
						GEngine->AddOnScreenDebugMessage(0, 3600, FColor::Red, TEXT("spawn!!!!"));
						UE_LOG(LogTemp, Warning, TEXT("spawn!!!! %d"), PC->LocalPlayerNum);
					}
					else
					{
...
void ALyraGameMode::HandleStartingNewPlayer_Implementation(APlayerController* NewPlayer)
{
	...
	{
		GEngine->AddOnScreenDebugMessage(-1, 3600.0f, FColor(255, 48, 16), TEXT("SPAWN PLAYER"));
